...
if [ -z "$INPUT_ZIP" ] || [ -z "$OUTPUT_ZIP" ]; then
    ...
    echo "Example: $0 videos_all.zip videos_all_720p_test.zip"
    exit 1
fi
...
echo "Testing directories: $TEST_DIR_1, $TEST_DIR_2" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"

# Get list of all video files from ONLY the two test directories (skip ._ metadata files)
echo "Analyzing archive for test directories..." | tee -a "$LOG_FILE"
VIDEO_LIST=$(unzip -Z1 "$INPUT_ZIP" | grep -E '\.(MOV|mov|mp4|MP4)$' | grep -v '/\._' | grep -E "^($TEST_DIR_1|$TEST_DIR_2)/")
...
echo "Sample files to be processed:" | tee -a "$LOG_FILE"
echo "$VIDEO_LIST" | head -5 | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"
